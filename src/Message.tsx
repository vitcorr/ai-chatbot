...
const Message = ({ messages }: MessageProps) => {
  return (
    <div className="flex flex-col bg-[#707d7d] rounded-2xl p-2 mb-50 mt-20">
      ...
      {messages.map((item, index) =>
        item.role == "user" ? (
          <div className="flex max-w-full items-end">
            ...
            <div
              className=" m-2 bg-[#00df81] px-4 py-2 rounded-2xl"
              key={index}
            >
          ...
          <div className="flex">
            ...
            <div className="m-2 bg-[#aacbc4] px-4 py-2 rounded-2xl" key={index}>
              <ReactMarkdown>{item.parts && item.parts[0].text}</ReactMarkdown>
            </div>
          </div>
        )
